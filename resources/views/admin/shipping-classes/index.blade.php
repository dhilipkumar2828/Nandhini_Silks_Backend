...
<div class="card-glass p-6 rounded-2xl">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div class="flex items-center space-x-4">
            <div>
                <h2 class="text-lg font-bold text-slate-800">Shipping Classes</h2>
                <p class="text-[10px] text-slate-400 font-medium">Group shipping rates for products</p>
            </div>
            <form method="GET" action="{{ route('admin.shipping-classes.index') }}" class="flex items-center">
                ...
                <select name="per_page" onchange="this.form.submit()" class="bg-slate-50 border-none rounded-lg px-2 py-1 text-[10px] font-bold text-slate-500 focus:ring-0 cursor-pointer">
                    <option value="10" {{ request('per_page') == 10 ? 'selected' : (!request('per_page') ? 'selected' : '') }}>10 rows</option>
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto items-center">
            <form action="{{ route('admin.shipping-classes.index') }}" method="GET" class="relative w-full sm:w-64">
                @if(request('status'))
                ...
                @if(request('search'))
                    <a href="{{ route('admin.shipping-classes.index', ['status' => request('status'), 'per_page' => request('per_page')]) }}" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-rose-500">
                ...
                <div class="relative">
                    <select onchange="window.location.href=this.value" 
                        ...
                        @foreach($statuses as $key => $label)
                            <option value="{{ route('admin.shipping-classes.index', ['status' => $key, 'search' => request('search'), 'per_page' => request('per_page')]) }}" {{ $currentStatus == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <a href="{{ route('admin.shipping-classes.create') }}" class="bg-[#a91b43] text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-[#940437] transition-all whitespace-nowrap shadow-sm">
