...
<div class="max-w-4xl mx-auto">
    <div class="card-glass p-8 rounded-2xl shadow-sm">
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-xl font-bold text-slate-800">Edit Shipping Rate</h2>
        ...
        <form action="{{ route('admin.shipping-rates.update', $shippingRate->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="shipping_class_id" required class="w-full bg-slate-50/50 border border-slate-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#a91b43]/20 focus:border-[#a91b43] transition-all">
                        ...
                        <option value="{{ $class->id }}" {{ old('shipping_class_id', $shippingRate->shipping_class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $shippingRate->name) }}" placeholder="e.g. Delivery Charge" required
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="country" value="{{ old('country', $shippingRate->country) }}" placeholder="e.g. India, All"
                           class="w-full bg-slate-50/50 border border-slate-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#a91b43]/20 focus:border-[#a91b43] transition-all">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="state" value="{{ old('state', $shippingRate->state) }}" placeholder="e.g. Tamil Nadu, All"
                           class="w-full bg-slate-50/50 border border-slate-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#a91b43]/20 focus:border-[#a91b43] transition-all">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="zip" value="{{ old('zip', $shippingRate->zip) }}" placeholder="e.g. 600001, All"
                           class="w-full bg-slate-50/50 border border-slate-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#a91b43]/20 focus:border-[#a91b43] transition-all">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" step="0.01" name="cost" value="{{ old('cost', $shippingRate->cost) }}" placeholder="0.00" required
            ...
            <div class="space-y-2">
                ...
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="1" {{ old('status', $shippingRate->status) == 1 ? 'checked' : '' }} class="text-[#a91b43] focus:ring-[#a91b43]">
                        <span class="text-xs font-bold text-slate-600">Active</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="0" {{ old('status', $shippingRate->status) == 0 ? 'checked' : '' }} class="text-slate-400 focus:ring-slate-400">
