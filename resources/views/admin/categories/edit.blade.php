        ...
        <form id="categoryForm" action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="slug" id="categorySlug" value="{{ old('slug', $category->slug) }}" required
                        class="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg text-sm outline-none focus:border-[#a91b43] focus:ring-2 focus:ring-pink-50 transition-all text-slate-800">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="display_order" value="{{ old('display_order', $category->display_order) }}" required min="0"
                        class="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg text-sm outline-none focus:border-[#a91b43] focus:ring-2 focus:ring-pink-50 transition-all text-slate-800">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex bg-slate-100 p-1 rounded-xl w-fit">
                        <label class="relative flex-1">
                            <input type="radio" name="status" value="1" class="sr-only peer" {{ old('status', $category->status) == '1' ? 'checked' : '' }}>
                        ...
                        <label class="relative flex-1">
                            <input type="radio" name="status" value="0" class="sr-only peer" {{ old('status', $category->status) == '0' ? 'checked' : '' }}>
...
<script>
    $(document).ready(function() {
        ...
        function checkSlug(slug) {
            ...
            timer = setTimeout(() => {
                $.get('{{ route("admin.categories.check-slug") }}', { slug: slug, id: currentId }, function(data) {
